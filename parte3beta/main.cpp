...
int main()
{
    ...
    auto h13 = new TH1F("h13", "K* extracted from invariant masses", 400, 0, 5);
    h13->Add(h10, h11, 1, -1);
    auto fit5a = new TF1("Fit subtraction discordant/concordant Pion Kaon", "gaus", 0.60, 1.30); //relativo fit
    h13->Fit(fit5a);

    auto h13b = new TH1F("h13b", "K* extracted from invariant masses", 5000, 0.5, 1.5);
    h13b->Add(h10b, h11b, 1, -1);

    auto fit5 = new TF1("Fit subtraction discordant/concordant Pion Kaon", "gaus", 0.7, 1.20); //relativo fit
    h13b->Fit(fit5);
    h13->Draw();
